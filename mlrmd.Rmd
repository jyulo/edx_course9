...
```
**Table 2: A list of the dataset movie titles, arranged from most to least rated**
...
$$1/(\lambda + n_i)$$
Where $\lambda$ is the term, and $n_i$ is the sample size of the group $i$ within a given predictor variable. For a given $\lambda$, as $n_i$ increases $\lambda + n_i$ approximates $n_i$ alone, and the penalty term becomes less significant.

As an example, a regularized first predictor term $b_1$ for value $i$ would look like this:
$$\hat{b_{1,i}}(\lambda) = \frac{1}{\lambda + n_i}\sum(Y_{1,i} - \hat{\mu})$$
As seen in the formula, larger values of $\lambda$ shrinks the estimate $\hat{b_{1,i}}$ further, which translates to a heavier penalization at a given $n_i$.
...
The code chunks displayed below are only included here to complete the methods section; tuning some of the later models with the genre matrices already saved resulted R running out of memory. Functional copies of the chunks below are included in the Results section, under the section for Model 3b.
...
*(Refer to the Methods section for more explanation on the following code.)*
